...
def main():
    ...
    # parse the input
    iter = []
    for subiter_string in iter_string.replace(" ","").split(";"):
        if len(subiter_string) == 0:
            continue
        temp = [ s.replace("[","").replace("]","") for s in subiter_string.split("][") ]
        subiter = []
        for elem in temp:
            subiter += [ Weight([ int(i) for i in elem.split(",") ]) ]
        iter += [subiter]
    ...
    for subiter in iter:
        # Start with the first irrep and an initial path (just the first irrep).
        path_list = [(subiter[0], str(subiter[0]))]  # Each element is a tuple: (current_irrep, path_string)

        # Process the remaining irreps in the subiter.
        for i in range(1, len(subiter)):
            new_path_list = []
            current_irrep = subiter[i]
            # For every current path, decompose the current irrep with the new one.
            for irrep, path_str in path_list:
                decomp = Decomposition(irrep, current_irrep)
                if (str(irrep), str(current_irrep)) in cg_count:
                    cg_count[(str(irrep), str(current_irrep))]+=1
                else:
                    cg_count[(str(irrep), str(current_irrep))] =1
                for result_irrep in decomp:
                    mult = decomp.multiplicity(result_irrep)
                    # For each occurrence, add the new step to the path.
                    for m in range(mult):
                        # Append the new step to the path: " x current_irrep -> result_irrep;m"
                        result_str = str(result_irrep).replace("]","; ")+str(m)+"]"
                        new_path = path_str + " × " + str(current_irrep) + " → " + result_str
                        new_path_list.append((result_irrep.copy(), new_path))
            path_list = new_path_list

        # After processing all factors, store each full path in the result dictionary.
        for _, full_path in path_list:
            decomp_result.append(full_path)

    # Print out all paths with their multiplicities.
    #for path in decomp_result:
    #    jsprint(path)

    # Print out list of occured CGs
    for key in cg_count:
        if cg_count[key]>1:
            jsprint(key[0]+"×"+key[1]+" occurs "+str(cg_count[key])+" times.")
        else:
            jsprint(key[0]+"×"+key[1]+" occurs 1 time.")
    ...
    for i,w3 in enumerate(decomp):
        ...
        for m in range(mult):
            irrep = str(w3).replace("[","").replace("]","").replace(",","")
            jsprint(irrep,m)
    '''
